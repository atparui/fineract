...
public class TenantDatabaseUpgradeService implements InitializingBean {
    ...
    private void upgradeTenantStore() throws LiquibaseException {
        if (fineractProperties.getTenant().isTenantStoreManagedExternally()
                || "tenant-service-api".equals(fineractProperties.getTenant().getResolution())) {
            log.info("Tenant store is managed externally or resolution is via tenant-service API; skipping Liquibase on tenant datasource.");
            return;
        }
